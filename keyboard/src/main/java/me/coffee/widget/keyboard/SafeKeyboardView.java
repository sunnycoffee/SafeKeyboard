...
public class SafeKeyboardView extends KeyboardView {
    ...
    private void drawSpecialKey(Canvas canvas, Keyboard.Key key) {
        if (key.codes[0] == -5) {
            ...
            if (logoDrawable != null) drawKeyBackground2(logoDrawable, canvas, key);
            // drawTextAndIcon(canvas, key, null, Color.WHITE);
    ...
    private void drawKeyBackground(int id, Canvas canvas, Keyboard.Key key) {
        ...
        drawable.setBounds(
                ...
                key.y + getPaddingTop(),
                key.x + key.width + getPaddingStart(),
    ...
    private void drawKeyBackground2(Drawable drawable, Canvas canvas, Keyboard.Key key) {
        ...
        drawable.setBounds(
                key.x + getPaddingStart() + 15,
                key.y + getPaddingTop() + 15,
                key.x + key.width + getPaddingStart() - 15,
                key.y + key.height + getPaddingTop() - 15);
        drawable.draw(canvas);
    }
    ...
    private void drawTextAndIcon(Canvas canvas, Keyboard.Key key, @Nullable Drawable drawable, int color) {
        try {
            ...
            if (key.label != null) {
                ...
                canvas.drawText(key.label.toString(), key.x + ((key.width) / 2 + +getPaddingStart()),
    ...
    private void setIconSize(Canvas canvas, Keyboard.Key key, int iconSizeWidth, int iconSizeHeight) {
        int left = key.x + (key.width - iconSizeWidth) / 2 + getPaddingStart();
        int top = key.y + (key.height - iconSizeHeight) / 2 + getPaddingTop();
        int right = key.x + (key.width + iconSizeWidth) / 2 + getPaddingStart();
